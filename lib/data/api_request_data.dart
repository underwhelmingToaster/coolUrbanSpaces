...
class ApiRequestData implements DataProvider {
  ...
  Future<List<SuggestionModel>> getAllSuggestions() async {
    ...
    if (response.statusCode == 200) {
      ...
      return suggestionList
          .map((dict) => SuggestionModel.fromJson(dict))
          .toList();
    } else {
      throw Exception('Failed to load all suggestions. Status: ' +
  ...
  Future<SuggestionModel> getSuggestion(int id) async {
    final response =
        await http.get(Uri.http(BASE_URL, '/api/suggestion/' + id.toString()));
    if (response.statusCode == 200) {
      return SuggestionModel.fromJson(jsonDecode(response.body));
    } else {
      throw Exception('Failed to load single suggestion. Status: ' +
          response.statusCode.toString());
    }
  }
  ...
  Future<void> postSuggestion(SuggestionModel suggestion) async {
    final response = await http.post(Uri.http(BASE_URL, '/api/suggestion/add'),
        body: jsonEncode(suggestion.toJson()),
        headers: {"Content-Type": "application/json"});
    if (response.statusCode == 200) {
      return;
    } else {
      throw Exception('Failed to post new Suggestion. Status: ' +
          response.statusCode.toString());
    }
  }
  ...
  Future<void> postMessage(MessageModel message) async {
    final response = await http.post(Uri.http(BASE_URL, '/api/message/add'),
